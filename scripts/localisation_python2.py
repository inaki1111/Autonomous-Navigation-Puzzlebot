...
class Localisation():  
    def __init__(self):
        ...
        rospy.Subscriber("whoIDIam", Int32, self.id_cb)
        rospy.Subscriber("distanceFromRobot", Float32, self.distance_cb)
        rospy.Subscriber("angleFromRobot", Float32, self.angle_cb)
        ...
        self.miu = np.array([2.9, 1.6, -np.pi/2])
        self.wr_k = 0.15
        self.wl_k = 0.025
    ...
    def id_cb(self, msg):
        id_aruco = msg.data
        print("Numero de aruco " + str(id_aruco))
        self.id_arucos(id_aruco)
        self.get_aruco=True

    def distance_cb(self, msg):
        self.d_aruco = msg.data

    def angle_cb(self, msg):
        self.theta_aruco = msg.data
    ...
    def correction_step(self):
        ...
        print("pos anterior " + str(self.miu))
        print("cov anterior" + str(self.z_covariance))
        
        self.delta_x = self.x_aruco - self.miu[0]
        self.delta_y = self.y_aruco - self.miu[1]
        self.p = self.delta_x ** 2 + self.delta_y ** 2

        print("coordenada x del aruco: " + str(x_aruco))
        print("coordenada y del aruco " + str(y_aruco))
        ...
        self.R_k = np.array([[0.045, 0], #error distance -0.065 , error angle 0.1 
                            [0, 0.115]])
