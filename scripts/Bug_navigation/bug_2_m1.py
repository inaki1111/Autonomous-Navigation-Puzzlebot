...
class AutonomousNav():  
	def __init__(self):
		...
		self.d_fw = 0.25  # distance from closest obstacle to activate the avoid obstacle behavior [m] 
		...
		self.distance_to_imaginary_line = 0.3 # distancia respecto a posicion actual del robot con linea imaginaria ax+by+c=0
		...
		self.a, self.b, self.c = self.calculate_line_params(3, 3, self.x_target, self.y_target)
	...
	def following_wallc(self): # FUNCION PARA FOLLOW WALL EN SENTIDO DE MANECILLAS DEL RELOJ
		...
		vfw = 0.1 # CONSTANTE DE VELOCIDAD LINEAL
		kw = 4.2 # GANANCIA PARA GIRO EN W
		wfw = kw * wfwc
		return vfw, wfw
	...
	def clear_shot(self): # FUNCION PARA VERIFICAR POSICION ACTUAL DEL ROBOT RESPECTO A LA LINEA AX+BY+C
		...
		print("current p " + str(current_distance) + " actual " + str(d_GTG) + "anterior dis " +str(self.dGTG_h))
