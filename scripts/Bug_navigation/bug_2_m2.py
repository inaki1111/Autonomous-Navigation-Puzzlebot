...
class AutonomousNav():  
	def __init__(self):
		...
		self.target_position_tolerance = 0.46  # acceptable distance to the goal to declare the robot has arrived to it [m] 
		
		self.d_fw = 0.4  # distance from closest obstacle to activate the avoid obstacle behavior [m] 
		...
		self.distance_to_imaginary_line = 0.42 # distancia respecto a posicion actual del robot con linea imaginaria ax+by+c=0
	...
	def following_wallc(self): # FUNCION PARA FOLLOW WALL EN SENTIDO DE MANECILLAS DEL RELOJ
		...
		kw = 2.9 # GANANCIA PARA GIRO EN W
		wfw = kw * wfwc
		return vfw, wfw
